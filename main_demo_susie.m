...
for dbi = 1 :length(db)
 ...
 elseif strcmp(db_name, 'Food')
        ...
        XTest = image_vgg(1:QUERY_SIZE,:);
        YTest = ingredient(1:QUERY_SIZE,:);
        LTest = label(1:QUERY_SIZE,:);
        
        XTrain = image_vgg(QUERY_SIZE + 1:QUERY_SIZE + TRAINING_SIZE,:);
        YTrain = ingredient(QUERY_SIZE + 1:QUERY_SIZE + TRAINING_SIZE,:);
        LTrain = label(QUERY_SIZE + 1:QUERY_SIZE + TRAINING_SIZE,:);
        
        XRetri = image_vgg(QUERY_SIZE + 1:QUERY_SIZE + DATABASE_SIZE,:);
        YRetri = ingredient(QUERY_SIZE + 1:QUERY_SIZE + DATABASE_SIZE,:);
        LRetri = label(QUERY_SIZE + 1:QUERY_SIZE + DATABASE_SIZE,:);

        clear image_vgg label ingredient    
    ...
    for ii =1:length(loopnbits)
        ...
        for jj = 1:length(hashmethods)
            ...
            switch(hashmethods{jj})
                  case 'TORCH'
                    fprintf('......%s start...... \n\n', 'TORCH');
                    param.eta1 = 100; param.eta2 = 0.2; param.eta3 = 1-param.eta2; param.eta4 = 10 ; param.eta5 = 100;
                    param.iter = 5;
                    param.rho = 1e-5;
                    OURparam = param;
    ...
    for ii = 1:length(loopnbits)
        for jj = 1:length(hashmethods)
            ...
            Table_ItoT_MAP_penalty(jj,ii) = eva_info{jj,ii}.Image_VS_Text_MAP_penalty;
            Table_TtoI_MAP_penalty(jj,ii) = eva_info{jj,ii}.Text_VS_Image_MAP_penalty;
    ...
    save(result_name,'eva_info','param','loopnbits','hashmethods',...
       ...
       'Table_ItoT_MAP_penalty','Table_TtoI_MAP_penalty','trainT',...
        '-v7.3');
end
